...
import numpy as np
import h5py
from gui.steering_trajectory import draw_trajectory_annotation
...
#get video metadata without loading full array
def get_video_metadata(file_path):
    with h5py.File(file_path, 'r') as f:
        vid_ds = f['X']  #access the dataset inside the file
        shape = vid_ds.shape
        frame_count = shape[0]
        frame_shape = shape[1:]  #c, h, w
    return frame_count, frame_shape

#load single frame from open h5 file handle
def load_frame_from_file(file_handle, idx):
    return np.asarray(file_handle['X'][idx])

#helper function to return video file as numpy array in shape F x C x H x W
#old function not used as it loads entire video frame into memory but kept for backwards compatibility, but not used in new implementation
...
#load telemetry but also return frame count for alignment
def load_telemetry(file_path, labels, fps=FPS_DEFAULT):
    ...
    with h5py.File(file_path,'r') as f: #open file
        ...
        #extract speed_abs as speed feature if available
        if 'speed_abs' in f:
            raw_speed = np.asarray(f['speed_abs'])
            tel['speed_abs'] = align_telemetry(raw_speed, ptr)

    #calculate frame count from telemetry alignment (like in dataset_loader)
    frame_max = max(v.shape[0] for v in tel.values()) if tel else 0
    tel['times'] = np.arange(frame_max)/fps #handle time here
    
    return tel, frame_max

#load camera file handle and telemetry - new implementation
def load_camera_and_labels(camera_path):
    ...
    #load telemetry data first to get frame count (like in dataset_loader)
    telemetry, frame_count_from_telemetry = load_telemetry(label_path, TELEMETRY_LABELS)
    
    #open h5 file and keep handle open
    video_file_handle = h5py.File(camera_path, 'r')
    
    #get video metadata to verify frame count matches
    vid_ds = video_file_handle['X']
    frame_count_from_video = vid_ds.shape[0]
    frame_shape = vid_ds.shape[1:]  #c, h, w
    
    #use the minimum frame count to ensure we don't go out of bounds
    frame_count = min(frame_count_from_video, frame_count_from_telemetry)
    
    return video_file_handle, telemetry, frame_count, frame_shape

#get total frames - updated to handle both old and new approaches
def get_frame_count(frames_or_count):
    if isinstance(frames_or_count, int):
        return frames_or_count  #new approach: direct count
    else:
        return frames_or_count.shape[0] #old approach: frames array

#get frame at idx as h x w x c uint8 - updated to handle file handle
def get_frame(frames_or_handle, idx, frame_count=None):
    #determine if using old or new approach
    if hasattr(frames_or_handle, 'keys') and 'X' in frames_or_handle.keys():  #h5 file handle
        #new approach: load frame on-demand
        if frame_count is None:
            #get frame count from h5 file dataset
            frame_count = frames_or_handle['X'].shape[0]
        
        idx = max(0, min(idx, frame_count-1)) #ensure index is within range
        
        try:
            frame_data = load_frame_from_file(frames_or_handle, idx)
            frame = frame_data.transpose(1,2,0) #transpose to H x W x C from C x H x W
            return np.clip(frame,0,255).astype(np.uint8) #cast as uint8 for memory efficiency
        except Exception as e:
            #fallback to black frame if loading fails
            return np.zeros((432, 864, 3), dtype=np.uint8)
    else:
        #old approach: index pre-loaded array
        idx = max(0,min(idx,get_frame_count(frames_or_handle)-1)) #ensure index is within number of frames
        frame = frames_or_handle[idx].transpose(1,2,0) #transpose to H x W x C from C x H x W
        return np.clip(frame,0,255).astype(np.uint8) #cast as uint8 for memory efficiency
...
def apply_noise_augmentation(frame, intensity=0.0):
    ...
    #generate coloured noise that preserves image characteristics
...
#updated to handle both old and new approaches
def process_frame(frames_or_handle, idx, telemetry=None, true_angle=0.0, sim_angle=0.0, 
                  show_trajectory=False, light_value=0.0, dim_value=0.0, noise_value=0.0, frame_count=None):
    #get the base frame - handle both old and new approaches
    if hasattr(frames_or_handle, 'keys') and 'X' in frames_or_handle.keys():  #h5 file handle
        frame = get_frame(frames_or_handle, idx, frame_count)
    else:
        frame = get_frame(frames_or_handle, idx)
