...
import numpy as np
import gui_efficient.gui_backend as gb
...
class VideoGui:
    def __init__(self, root):
        ...
        #state vars - updated for new approach
        self.video_file_handle = None  #h5 file handle instead of frames array
        self.telemetry = None
        self.frame_count = 0
    ...
    def on_sim_driving_changed(self, control_type, value):
        ...
        if self.traj_var.get() == 1 and self.video_file_handle is not None:
    ...
    def load_file(self):
        ...
        if self.video_file_handle is not None:
            try:
                self.video_file_handle.close()
            except:
                pass
            self.video_file_handle = None
        
        try:
            #use new approach: get file handle and metadata
            self.video_file_handle, self.telemetry, self.frame_count, self.frame_shape = gb.load_camera_and_labels(path)
        except Exception as e:
            messagebox.showerror('Error', str(e))
            return
        
        #after loading a new file, reset the speed graph
        if hasattr(self, 'speed_graph_obj'):
    ...
    def slider_moved(self, val):
        if self.updating_slider or self.video_file_handle is None:
            return
    ...
    def model_play_next(self):
        ...
        #display the frame (this will apply trajectory visualisation if enabled)
        self.display_frame(self.current_idx)
    ...
    def update_saliency_display(self):
        #only update if enabled and file handle exists
        if self.saliency_var.get() == 1 and self.video_file_handle is not None:
            light_val = float(self.light_slider.get())
            dim_val = float(self.dim_slider.get())
            noise_val = float(self.noise_slider.get())
    ...
    def cleanup(self):
        #close video file handle if open
        if self.video_file_handle is not None:
            try:
                self.video_file_handle.close()
            except:
